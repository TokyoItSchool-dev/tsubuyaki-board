...
TokyoItSchool-dev/            TokyoItSchool-dev/tsubuyaki-board          <github-id>/tsubuyaki-board
...
| 公開リポの所有者権限 | upstream リポの公開・ブランチ保護設定 | 個人アカウント、または Org 管理者として `TokyoItSchool-dev/tsubuyaki-board` を作成・設定できる |
...
公開リポ `TokyoItSchool-dev/tsubuyaki-board` を置く場所を決めます。Organization は**任意**です（fork モデルでは受講生の招待が不要なため、Org の請求・メンバー管理は要りません）。
...
> 💡 以降このガイドでは upstream リポを `TokyoItSchool-dev/tsubuyaki-board` と表記します。配布時に実際の owner 名へ読み替えてください（受講生ガイドの `<owner>` と一致させる）。
...
gh repo create TokyoItSchool-dev/tsubuyaki-board \
    --public \
    --description "AI 駆動開発研修 (Codex × Spring Boot) 演習リポジトリ（受講生は各自 Fork して作業）"
```

> 💡 既存のリポを後から公開へ切り替える場合は `gh repo edit TokyoItSchool-dev/tsubuyaki-board --visibility public`（または Settings → 最下部 Danger Zone → Change visibility → Public）。
...
```bash
gh repo view TokyoItSchool-dev/tsubuyaki-board --json visibility,defaultBranchRef
```
...
`gh` で設定する例（PR 必須＝直接 push 禁止の最小構成。JSON を `--input` で渡す）(Git Bashまたはubuntsuで実行):

```bash
cat <<'JSON' | gh api -X PUT "repos/TokyoItSchool-dev/tsubuyaki-board/branches/main/protection" --input -
{
...
- **研修リポ（upstream）URL** `https://github.com/TokyoItSchool-dev/tsubuyaki-board`
...
2. 受講生ガイド §3 と同じく、upstream `https://github.com/TokyoItSchool-dev/tsubuyaki-board` を開き、右上の「**Fork**」でテストアカウントのアカウントへ Fork する
...
gh pr create --repo TokyoItSchool-dev/tsubuyaki-board \
...
- 研修リポ（upstream）URL（`https://github.com/TokyoItSchool-dev/tsubuyaki-board`）を配布（Slack / メール / ホワイトボード）。「各自 Fork して使う」ことを明示。招待・承認の作業は無い
- Pleiades 配布媒体を回覧
...
- 原因: upstream が public になっていない、clone 先が upstream（`TokyoItSchool-dev/...`）のまま、PAT のスコープ／失効
...
### 公開リポ / ブランチ保護
- [ ] `TokyoItSchool-dev/tsubuyaki-board` を **public** で作成済（§3）
...
- [ ] 研修リポ（upstream）URL（`https://github.com/TokyoItSchool-dev/tsubuyaki-board`）を受講生案内に記載（「各自 Fork」の旨も）
...
- **PR を close し、upstream はそのまま次期研修へ再利用**: upstream の `main` は無傷なので、受講生の Draft PR を close するだけで次期研修にそのまま使えます。受講生の fork は各自の所有物なので、講師が消す必要はありません。
  ...
  gh pr list --repo TokyoItSchool-dev/tsubuyaki-board --state open

  # 個別に close（マージはしない）
  gh pr close --repo TokyoItSchool-dev/tsubuyaki-board <PR番号>
  ```
...
- **upstream リポごと削除**: 次期研修で作り直す場合。
  ...
  ```bash
  gh repo delete TokyoItSchool-dev/tsubuyaki-board --yes
  ```
