...
2. 当該受講生に `git reflog` で force push 前の HEAD を確認させ、`git reset --hard <SHA>` でローカルを巻き戻させる。
3. もう一度 `git push --force-with-lease`（講師立会いの上で 1 回限り許可）して main を復旧。
4. AGENTS.md §7.3.3「履歴・権限改ざん禁止」を全体に再共有。
